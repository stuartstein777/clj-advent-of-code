...
(defn update-file-structure [[files dirs] current-director file-system]
...
(defn process-commands [stack file-structure [cmd & rcmds]]
  ...
  (if  (seq cmd)
    (cond
      ...
      (recur (rest stack) file-structure rcmds)
      ...
      (let [listings (get-listings rcmds)
            ...
            updated-file-structure (update-file-structure files-dirs (last stack))]
        (prn files-dirs)
        (tap> files-dirs)
        (recur stack file-structure (drop (count listings) rcmds)))
...
{"/" {:dirs #{"a"} :files #{{:name "b.txt" :size 14848514}
